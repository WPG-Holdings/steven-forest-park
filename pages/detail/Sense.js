...
export default function Sense() {
  ...
  const getBedDetail = async () => {
    try {
      const data = await api.detail.getBedDetail(193);
  ...
  const getFabricDetail = async () => {
    try {
      const data = await api.detail.getFabricDetail(317);
  ...
  return (
    <>
      <HeadMeta
        title={'東璟家具 - 床架 - Sense'}
        description={
          '在思維降落的地方躺下，實踐一場浪漫的沈睡；或者在恰到好處的時間清醒，開始探索美。'
        }
        keywords={
          '家具,精品床,床架,睡眠,雙人床,雙人加大,雙人特大,理性與感性,sense'
        }
      />
      {detailData && (
        <div className="page-sense">
