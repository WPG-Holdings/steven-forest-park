...
export default function Minutiae() {
  ...
  const getSofaDetail = async () => {
    try {
      const data = await api.detail.getSofaDetail(168);
  ...
  const getFabricDetail = async () => {
    try {
      const data = await api.detail.getFabricDetail(370);
  ...
  return (
    <>
      <HeadMeta
        title={'東璟家具 - 沙發 - Minutiae'}
        description={
          '小至線索、大至潮流，細節，是復興這個時代面貌的基本單位。當流行有所保留，只有我總是前進。'
        }
        keywords={'家具,精品沙發,沙發,細節的復興,minutiae'}
      />
      {detailData && (
        <div className="page-minutiae">
