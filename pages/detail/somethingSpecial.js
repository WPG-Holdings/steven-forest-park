...
export default function somethingSpecial() {
  ...
  const getSofaDetail = async () => {
    try {
      const data = await api.detail.getSofaDetail(464);
  ...
  const getFabricDetail = async () => {
    try {
      const data = await api.detail.getFabricDetail(465);
  ...
  return (
    <>
      <HeadMeta
        title={'東璟家具 - 沙發 - Something/Special'}
        description={
          '生活的性情是高昂的，抬起頭、翹起腳、舉起手腕與蔻丹，我是我，四面八方完美。'
        }
        keywords={'家具,精品沙發,沙發,自傲形式,Something/Special'}
      />
      {detailData && (
